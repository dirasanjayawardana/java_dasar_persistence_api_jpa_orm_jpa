...
public class CriteriaTest {
    ...
    void criteriaQuery() {
        ...
        Root<Brand> b = criteria.from(Brand.class); // Root seperti membuat alias di JPA QL
    ...
    void criteriaWhereClause() {
        ...
        // jika tidak menyebutkan builder.or/and, maka defaultnya AND
        criteria.where(
    ...
    void criteriaAggregateQuery() {
        ...
        // select b.id, min(p.price), max(p.price), avg(p.price) from Product p join
        // p.brand b

        criteria.groupBy(b.get("id"));
        // select b.id, min(p.price), max(p.price), avg(p.price) from Product p join
        // p.brand b group by b.id

        criteria.having(builder.greaterThan(builder.min(p.get("price")), 500_000L));
        // select b.id, min(p.price), max(p.price), avg(p.price) from Product p join
        // p.brand b group by b.id having min(p.price) > 500000
